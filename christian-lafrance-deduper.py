...
def get_args():
    ...
    parser.add_argument("-o", help="Designates absolute file path to sorted sam file", required=False, type=str)
    parser.add_argument("-u", help="Designates file containing the list of UMIs", required=False, type=str)
...
def get_UMI(qname:str) -> str:
    '''
    Takes in a SAM file line and returns the UMI from the end of the QNAME. 
    '''
...
def check_trie(trie: dict, tup: tuple) -> bool:
    ...
    the same UMI and strand but have a different position. Returns a 
    boolean representing if the umi-strand-position combination is 
    unique. 
    '''
